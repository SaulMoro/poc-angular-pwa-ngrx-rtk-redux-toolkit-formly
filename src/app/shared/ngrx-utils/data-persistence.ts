  ...
  return (source: ActionStateStream<T, A>): Observable<Action> => {
    if (opts.id) {
      ...
      return groupedFetches.pipe(
        mergeMap((pairs) => pairs.pipe(switchMap(runWithErrorHandling(opts.run, opts.onError))))
      );
    }
...
// tslint:disable-next-line: typedef
function runWithErrorHandling<T, A, R>(run: (a: A, state?: T) => Observable<R> | R | void, onError: any) {
